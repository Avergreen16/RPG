...
struct Player {
    ...
    Player() = default;
    Player(Core* core_ptr, std::array<double, 2> position, uint texture_id, std::string name);
...
struct Core {
    ...
    std::queue<uint> chunk_mesh_gen_queue;
    std::thread chunk_mesh_gen_thread;
    ...
    */
    ...
    Core(GLFWwindow* window, netwk::TCP_client& connection_ref, std::array<double, 2> player_start_pos, std::string name, bool& game_running);
