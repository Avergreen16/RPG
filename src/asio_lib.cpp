                            ...
                            if(error_code) { // if there is an error, disconnect player and tell other clients
                                ...
                                switch(header_type) {
                                    case 0: { // player logged in
                                        player_join_packet_toserv recv_packet = convert_byte_vector<player_join_packet_toserv>(buffer_body->data());
                                    ...
                                    case 1: { // chat message
                                        std::string message = this->player_name + " \\c000: " + make_string(reinterpret_cast<char*>(buffer_body->data()), bytes_transferred);
                                        std::vector<uint8_t> body(message.size());
                                        memcpy(body.data(), message.data(), message.size());
                                        parent_server->broadcast(0, body);
